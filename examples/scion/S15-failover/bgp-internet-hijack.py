...
def create_as(isd, asn, is_core=False, issuer=None):
    ...
    cs = as_.createControlService('cs1').joinNetwork('net0')
    cs.addSoftware("iperf3")
    cs.addSoftware("traceroute")
...
cs180 = base.getAutonomousSystem(180).getControlService('cs1')
cs180.addSharedFolder('/dmtp', '../dmtp_client')
cs180.addSoftware('python3')
cs180.addSoftware('python3-pip')
cs180.addBuildCommand('pip3 install scapy')

cs200 = base.getAutonomousSystem(200).getControlService('cs1')
cs200.addSharedFolder('/dmtp','../dmtp_server')
# cs200.addSoftware('python3')
# cs200.addSoftware('python3-pip')
# cs200.addBuildCommand('pip3 install scapy')
...
ebgp.addCrossConnectPeering(60, 152, PeerRel.Unfiltered)
...
scion.addXcLink((1, 200), (1, 163), ScLinkType.Transit)
scion.addXcLink((1, 200), (1, 150), ScLinkType.Transit)
for a, b in [(a, b) for a in peers_ix12 for b in peers_ix12 if a < b]:
    scion.addIxLink(12, (1, a), (1, b), ScLinkType.Transit)
for a, b in [(a, b) for a in peers_ix11 for b in peers_ix11 if a < b]:
    scion.addIxLink(11, (1, a), (1, b), ScLinkType.Transit)
for a, b in [(a, b) for a in peers_ix10 for b in peers_ix10 if a < b]:
    scion.addIxLink(10, (1, a), (1, b), ScLinkType.Transit)
...
# Compilation
emu.compile(Docker(), './bgp-hijack', override=True)
emu.compile(Graphviz(), "./bgp-hijack/graphs", override=True)
